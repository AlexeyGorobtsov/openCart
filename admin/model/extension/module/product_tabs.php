  ...
  public function updateTab($module_categories, $extra_tab_id, $tab_description) {
    ...
    if ($results) {
      foreach ($results as $product_id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_extra_tabs WHERE product_id = '" . (int)$product_id . "' AND extra_tab_id = '" . (int)$extra_tab_id . "'");

        foreach ($tab_description as $language_id => $text) {
          $this->db->query("INSERT INTO " . DB_PREFIX . "product_extra_tabs SET product_id = '" . (int)$product_id . "', extra_tab_id = '" . (int)$extra_tab_id . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($text['text']) . "'");
        }
      }
