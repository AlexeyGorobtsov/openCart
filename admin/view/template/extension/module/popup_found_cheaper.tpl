          ...
          <div class="tab-content">
            <div class="tab-pane active" id="setting-tab">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <input type="text" name="popup_found_cheaper_data[mask]" value="<?php echo (isset($popup_found_cheaper_data['mask'])) ? (!empty($popup_found_cheaper_data['mask']) ? $popup_found_cheaper_data['mask'] : '') : ''; ?>" placeholder="<?php echo $entry_mask_info; ?>" id="input-mask" class="form-control" />
                </div>
              </div>
            </div>
...
$('#history').load('index.php?route=extension/module/popup_found_cheaper/history&token=<?php echo $token; ?>');
function delete_selected(request_id) {
  $.ajax({
    type: 'post',
    url:  'index.php?route=extension/module/popup_found_cheaper/delete_selected&token=<?php echo $token; ?>&delete=' + request_id,
    dataType: 'json',
...
function delete_all() {
  $.ajax({
    type: 'get',
    url:  'index.php?route=extension/module/popup_found_cheaper/delete_all&token=<?php echo $token; ?>',
    dataType: 'json',
    success: function(json) {
      $('#history').load('index.php?route=extension/module/popup_found_cheaper/history&token=<?php echo $token; ?>');
    }
...
function delete_all_selected() {
  $.ajax({
    type: 'post',
    url:  'index.php?route=extension/module/popup_found_cheaper/delete_all_selected&token=<?php echo $token; ?>',
...
function update_note(request_id, note) {
  $.ajax({
    type: 'post',
    url:  'index.php?route=extension/module/popup_found_cheaper/update_note&token=<?php echo $token; ?>&request_id=' + request_id + '&note=' + note,
    dataType: 'json',
    success: function(json) {
      $('#history').load('index.php?route=extension/module/popup_found_cheaper/history&token=<?php echo $token; ?>');
    }
