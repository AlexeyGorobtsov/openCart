...
class ControllerExtensionModulePopupCallPhone extends Controller {
    ...
    $data['entry_mask_info'] = $this->language->get('entry_mask_info');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/popup_call_phone', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/module/popup_call_phone', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/popup_call_phone.tpl', $data));
	}
    ...
    $results = $this->model_extension_module_popup_call_phone->getCallArray($filter_data);
    ...
    $history_total = $this->model_extension_module_popup_call_phone->getTotalCallArray();

    $pagination = new Pagination();
    $pagination->total = $history_total;
    $pagination->page = $page;
    $pagination->limit = 20;
    $pagination->url = $this->url->link('extension/module/popup_call_phone/history', 'token='.$this->session->data['token'].'&page={page}', true);
    ...
    $this->response->setOutput($this->load->view('extension/module/popup_call_phone_history.tpl', $data));
  }
    ...
    $info = $this->model_extension_module_popup_call_phone->getCall((int)$this->request->get['delete']);

    if ($info) {
      $this->model_extension_module_popup_call_phone->deleteCall((int)$this->request->get['delete']);
    }
  ...
  public function delete_all() {
    ...
    $this->model_extension_module_popup_call_phone->deleteAllCallArray();
  ...
  public function delete_all_selected() {
    ...
    if (isset($this->request->request['selected'])) {
      foreach ($this->request->request['selected'] as $request_id) {
        $info = $this->model_extension_module_popup_call_phone->getCall((int)$request_id);

        if ($info) {
          $this->model_extension_module_popup_call_phone->deleteCall((int)$request_id);
        }
      }
    }
    ...
    $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'extension/module/popup_call_phone');
    $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'extension/module/popup_call_phone');

    $this->model_extension_module_popup_call_phone->makeDB();
  ...
  public function uninstall() {
    ...
    $this->model_extension_module_popup_call_phone->deleteDB();
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/popup_call_phone')) {
